...
// Lazy-init: resolve the connection string from any known env var name
// Neon Vercel integration may use DATABASE_URL, POSTGRES_URL, or others
var _sql = null;
function getSql() {
  if (_sql) return _sql;
  ...
  if (!url) {
    console.error('FDC _db.js: No DB URL found. Checked: DATABASE_URL, POSTGRES_URL, POSTGRES_URL_NON_POOLING, NEON_DATABASE_URL');
    console.error('FDC _db.js: Available env vars with DB/POSTGRES/NEON:', Object.keys(process.env).filter(function(k) { return k.indexOf('POSTGRES') >= 0 || k.indexOf('DATABASE') >= 0 || k.indexOf('NEON') >= 0; }));
    throw new Error('FDC _db.js: No database URL found. Set DATABASE_URL or POSTGRES_URL.');
  }
  console.log('FDC _db.js: Connecting (' + url.substring(0, 30) + '...)');
  _sql = neon(url, { fullResults: true });
  return _sql;
}

// Proxy so callers can use sql`...` as a tagged template literal
var sql = new Proxy(function(){}, {
